<?php
  ...
  $objPHPExcel->getActiveSheet()->getStyle('A1:F6')->applyFromArray($styleHeader);

  // Add Heaader of Report
  $objPHPExcel->getActiveSheet()->mergeCells('A1:F1');
  $objPHPExcel->getActiveSheet()->setCellValue('A1','รายงานด้าน Configuration Management');
  $objPHPExcel->getActiveSheet()->mergeCells('A2:F2');
  $objPHPExcel->getActiveSheet()->setCellValue('A2','ข้อมูลฮาร์ดแวร์');
  $objPHPExcel->getActiveSheet()->setCellValue('E3','วันที่ออกรายงาน:');
  ...
  $colHeader = array( "ลำดับ","วันที่รับเข้ามา","หมวดหมู่อุปกรณ์","หมายเลขอุปกรณ์","รายละเอียดฮาร์ดแวร์","ใช้ประจำที่");
  $rowNumber = 7;
  $column = 'A';
  ...
  while ($n = $result->fetch_array()) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('E'.$row,$n['equipment_HWdetail']);
    $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,$n['room_num']);
    $row++;
    $no++;
  }
  $maxrow = $row;
  ...
  $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth("11.38");
  $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth("14.88");
  $objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth("14.88");
  $objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth("23.38");
  $objPHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth("11.63");
  ...
  $objPHPExcel->getActiveSheet()->getStyle('A7:F'.$maxrow)->applyFromArray($styleArray);

  // Rename worksheet
  $objPHPExcel->getActiveSheet()->setTitle('HW');
 //
 ...
 //  $objPHPExcel->setActiveSheetIndex(0);
  ...
  header('Content-Disposition: attachment;filename="รายงานฮาร์ดแวร์.xlsx"');
